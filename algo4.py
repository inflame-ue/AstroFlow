...
class SimulateMission:
    ...
    def add_satellite(self, orbit: Orbit, angle: float, speed_factor: float):
        # Physics-based speed using MU and actual orbit radius
        speed = np.sqrt(MU / orbit.radius) * speed_factor  # Use provided factor
    ...
    def calculate_intercept_time(self, spacecraft1, spacecraft2):
        """Calculate time to intercept between two spacecraft in the same orbit"""
        ...
        if intercept_time < -1e-9: # If time is negative, add one period of relative phase
             # Relative period = 2*pi / |relative_speed|
    ...
    def find_next_optimal_target(self, current_orbit_idx):
        ...
        current_tanker_speed = self.tanker.speed # Speed in current circular orbit

        if abs(current_tanker_speed) < 1e-9:
             print("Warning: Tanker speed is near zero. Cannot calculate wait time accurately.")
             # Handle this case: maybe assume a default wait or fail gracefully
             # For now, let's return no target if tanker can't move in orbit
             return None, None, float('inf')

        for satellite in next_orbit.satellites:
            # Calculate current angular positions
            current_satellite_angle = satellite.angle % (2 * np.pi)
            satellite_speed = satellite.speed # Speed in target circular orbit

            # Calculate satellite's angle at the *end* of the Hohmann transfer
            # The arrival point of the tanker after transfer is pi radians ahead of departure point
            satellite_angle_at_arrival = (current_satellite_angle + satellite_speed * transfer.transfer_time) % (2 * np.pi)

            # The tanker needs to *arrive* at an angle pi radians *ahead* of the satellite's position *at arrival*
            # (Tanker arrival angle = satellite_angle_at_arrival + pi)
            # The transfer itself covers pi radians.
            # So, the tanker must *depart* from an angle equal to the satellite's angle *at arrival*.
            required_departure_angle = satellite_angle_at_arrival
            ...
            # Calculate wait time
            # wait_time = angle / speed
            wait_time = angle_to_travel / current_tanker_speed

            # Ensure wait time is positive (add full orbit period if negative)
            if wait_time < -1e-9: # Use tolerance
                orbit_period = (2 * np.pi * current_orbit.radius) / abs(transfer.v1 / BASE_ORBITAL_SPEED_FACTOR) if abs(transfer.v1) > 1e-9 else float('inf') # Time for tanker to orbit once
                orbit_period_sim_units = (2* np.pi) / abs(current_tanker_speed) if abs(current_tanker_speed) > 1e-9 else float('inf')
                # print(f"DEBUG: Negative wait time {wait_time:.2f}. Adding period {orbit_period_sim_units:.2f}")
                if orbit_period_sim_units == float('inf'):
                     print("Warning: Cannot calculate orbit period for wait time adjustment (zero speed?).")
                     continue # Skip this target if period is infinite
                wait_time += orbit_period_sim_units
                # Recalculate angle to travel based on adjusted wait time (should now be positive equivalent)
                # angle_to_travel = (wait_time * current_tanker_speed) % (2*np.pi)

            # Calculate total time for this option
        ...
        optimal_wait_time = max(0, optimal_wait_time)
    ...
    def find_next_shuttle_target(self, current_orbit_idx):
        ...
        for shuttle in next_orbit.shuttles:
            ...
            shuttle_speed = shuttle.speed # Speed in target circular orbit
    ...
    def execute_hohmann_transfer(self, transfer):
        """Executes a Hohmann transfer using Keplerian calculations."""
        ...
        if transfer.transfer_time <= 1e-9:
            ...
            # Set final state immediately if radii are the same
            self.tanker.radius = transfer.r2
            # Angle remains the same (or should be updated by wait phase)
            # Speed should be updated if r1=r2 but target speed is different (unlikely for Hohmann)
            self.tanker.speed = np.sqrt(MU / transfer.r2) * BASE_ORBITAL_SPEED_FACTOR if transfer.r2 > 0 else 0
            # Add final point which is same as start point but with potentially updated time
            transfer_trajectory_points.append((self.mission_clock, start_pos_x, start_pos_y))
            return transfer_trajectory_points

        # Transfer parameters
        a = transfer.a
        e = transfer.e # Use eccentricity from transfer object
        ...
        for i in range(1, steps + 1):
            ...
            frac = i / steps
            mean_anomaly = frac * np.pi  # M goes 0 to pi for half-orbit transfer

            # Newton-Raphson solver for Eccentric Anomaly (E)
            E = mean_anomaly  # Initial guess (good for small e)
            # For higher e, E = M + e*sin(M) might be better, but M is fine.
            ...
            if transfer.r1 < transfer.r2: # Outward (Periapsis -> Apoapsis, E: 0 -> pi)
                 x_prime = a * (np.cos(E) - e)
                 y_prime = a_sqrt_1_minus_e2 * np.sin(E)
            else: # Inward (Apoapsis -> Periapsis, E: effectively pi -> 2pi)
                 # Use E (0->pi) to calculate position for the second half of the ellipse
                 x_prime = a * (-np.cos(E) - e) # Equivalent to cos(E+pi)
                 y_prime = -a_sqrt_1_minus_e2 * np.sin(E) # Equivalent to sin(E+pi)
            ...
            # Time passes by actual_time_step for *this segment* of the transfer
            for satellite in self.satellites:
                 if satellite.active:
                     satellite.update(actual_time_step)
            for shuttle in self.shuttles:
                 if shuttle.active:
                     # Only update shuttle if it's currently deployed and not recovered
            ...
            # Advance mission clock *after* updating other objects for the interval
            self.mission_clock += actual_time_step

            # Append calculated inertial position with the time at the END of this step
            transfer_trajectory_points.append((self.mission_clock, pos_x, pos_y))

            # Update tanker's internal state (radius/angle) for the *next* potential calculation
            # These are intermediate values, final state set after loop
        ...
        # The last point in transfer_trajectory_points should correspond to the final state
        ...
        self.tanker.speed = np.sqrt(MU / self.tanker.radius) * BASE_ORBITAL_SPEED_FACTOR if self.tanker.radius > 0 else 0

        return transfer_trajectory_points


    def simulate_launch(self, tanker: Tanker, target_orbit: Orbit, target_angle: float):
        """Simulate launch from planet to first orbit (simple interpolation)"""
        ...
        # Estimate total launch time (arbitrary duration for visualization)
        estimated_launch_duration = 10.0 # Adjusted duration
        ...
        for i in range(1, steps + 1):
            ...
            # --- Update other objects during this time step ---
            # Calculate time elapsed in this specific step segment
            time_in_this_step = estimated_launch_duration / steps
            # Update objects based on state *before* this step's time progression
            for satellite in self.satellites:
                 if satellite.active:
                     satellite.update(time_in_this_step)
            for shuttle in self.shuttles:
                 if shuttle.active:
                     # Check deployment/recovery status relative to the *end* of this step time
            ...
            # Update mission time *after* updates
        ...
        tanker.speed = np.sqrt(MU / tanker.radius) * BASE_ORBITAL_SPEED_FACTOR if tanker.radius > 0 else 0
    ...
    def simulate_reentry(self, tanker: Tanker, target_landing_angle: float):
        """Simulate reentry from lowest orbit to planet surface (simple interpolation)"""
        ...
        # Estimate total reentry time (arbitrary duration for visualization)
        estimated_reentry_duration = 10.0 # Adjusted duration

        # --- NEW ANGLE LOGIC ---
        # Estimate angle change based on initial orbital speed and duration
        initial_orbital_speed = tanker.speed # Speed just before reentry starts
        estimated_angle_travel = initial_orbital_speed * estimated_reentry_duration
        ...
        for i in range(1, steps + 1):
            ...
            # current_angle = (start_angle + t * delta_angle) % (2 * np.pi) # OLD interpolation
            ...
            # --- Update other objects during this time step ---
            time_in_this_step = estimated_reentry_duration / steps
            for satellite in self.satellites:
                 if satellite.active:
                     satellite.update(time_in_this_step)
            for shuttle in self.shuttles:
    ...
    def deploy_shuttle_to_orbit(self, orbit: Orbit):
        """Deploy a shuttle to the current orbit"""
        ...
        orbital_speed_magnitude = np.sqrt(MU / orbit.radius) * BASE_ORBITAL_SPEED_FACTOR if orbit.radius > 0 else 0
        ...
        shuttle = self.deploy_shuttle(
            orbit,
            deployment_angle,
            -orbital_speed_magnitude # Opposite direction to standard prograde orbit
        )

        # Store deployment info
        shuttle.initial_angle = deployment_angle # Record angle at the moment of deployment
        shuttle.deployment_time = deployment_time # Record time of deployment
    ...
    def recover_shuttle(self, shuttle: Shuttle):
        ...
        # Mark shuttle as intercepted and inactive for animation/updates
        shuttle.intercepted = True
        shuttle.active = False # Stop updating its position
    ...
    def wait_in_orbit(self, wait_time):
        """Wait in current orbit for the specified time, updating all objects."""
        if wait_time <= 1e-9: # No need to wait if time is negligible
            return []

        start_angle = self.tanker.angle
        start_radius = self.tanker.radius # Should be constant during wait

        # Determine number of steps for trajectory based on wait time
        # More steps for longer waits or higher speeds for smoother visuals
        # Use a base time resolution + speed factor
        time_resolution = 0.5 # Sample point every 0.5 time units
        speed_factor_steps = abs(self.tanker.speed) * 10 # More steps if moving faster
        steps = max(1, int(wait_time / time_resolution + speed_factor_steps))

        # --- CAP THE STEPS --- #
        max_wait_steps = 1000 # Limit steps to prevent excessive calculation
        ...
        for i in range(steps):
            # Update satellites and active shuttles first for this time step
            for satellite in self.satellites:
                if satellite.active:
                    satellite.update(actual_time_step)

            for shuttle in self.shuttles:
                 if shuttle.active: # Checks deployed and not recovered implicitly
                     # Additional check for time window just in case
                     is_deployed = shuttle.deployed and shuttle.deployment_time is not None and (self.mission_clock + actual_time_step) >= shuttle.deployment_time
                     is_not_yet_recovered = shuttle.recovery_time is None or (self.mission_clock + actual_time_step) < shuttle.recovery_time
                     if is_deployed and is_not_yet_recovered:
                         shuttle.update(actual_time_step)
            ...
            # Update tanker position for this step
            self.tanker.update(actual_time_step)

            # Advance mission clock *after* updates
            self.mission_clock += actual_time_step
            ...
            wait_trajectory_points.append((self.mission_clock, current_pos_x, current_pos_y)) # Add point with updated time

        # Add event log entry if wait was significant
        current_orbit_idx = self.find_current_orbit_index()
        if wait_time > 0.1: # Log waits longer than 0.1 time units
            self.tanker.add_event(
                self.mission_clock,
                f"Waited in orbit {self.orbit_index_to_string(current_orbit_idx)} for {wait_time:.2f} time units"
            )
    ...
    def simulate_mission(self):
        ...
        launch_target_angle = 0.0 # Target angle 0 in the first orbit
        print(f"Calculating launch trajectory to orbit {lowest_orbit.radius:.1f} km, target angle {launch_target_angle:.2f} rad...")
    ...
    def visualize(self):
        ...
        def update(frame):
            ...
            # --- Interpolation Logic ---
            # Calculate target position in trajectory list based on frame
            target_traj_pos = (frame / (num_frames - 1)) * (num_trajectory_points - 1)
            idx0 = int(target_traj_pos)
            # Ensure indices are within bounds for interpolation (idx0 up to second-to-last)
            idx0 = min(max(0, idx0), num_trajectory_points - 2)
            idx1 = idx0 + 1

            # Calculate interpolation ratio (how far between idx0 and idx1)
            ...
            # Interpolate time and tanker position
            ...
            # Update satellite positions based on interpolated time
            for i, satellite in enumerate(self.satellites):
                if satellite.active: # Check if satellite is active (it usually is)
                    # Calculate position based on initial angle and elapsed time
                    ...
                    satellite_markers[i].set_data([], []) # Hide inactive satellites


            # Update shuttle positions based on interpolated time
            for i, shuttle in enumerate(self.shuttles):
                # Determine visibility based on deployment and recovery times vs interpolated time
                ...
                if is_deployed and not is_recovered:
                    ...
                    # Calculate position at interpolated mission time
                    ...
                    # shuttle_recovered_markers[i].set_data([], []) # Hide recovered marker
                # elif is_recovered:
                #     # Show recovered marker at last known position (or tanker position?)
                #     # For simplicity, just hide active marker and show recovered marker fixed
                #     # Need to store last position before recovery if we want it fixed
                #     # Find trajectory point closest to shuttle.recovery_time? Too complex for now.
                #     # Just hide recovered marker too.
                #     # shuttle_recovered_markers[i].set_data([shuttle_x_last], [shuttle_y_last])
                #     shuttle_recovered_markers[i].set_data([], [])
                else:
                    # Hide both markers if not deployed or already recovered
                    shuttle_active_markers[i].set_data([], [])
                    # shuttle_recovered_markers[i].set_data([], [])


            # Update tanker position using interpolated position
            # Check if tanker is still active (hasn't landed)
            if self.tanker.active or estimated_mission_time < self.tanker.mission_events[-1][0]: # Show until final event time
                 tanker_marker.set_data([current_tanker_x], [current_tanker_y])
            else: # Hide tanker after landing complete
                 tanker_marker.set_data([],[])
            ...
            time_text.set_text(f'Time: {estimated_mission_time:.1f}')

            # Update mission status text based on events
            status = "Pre-launch"
            # Find the latest event that occurred before or at the current interpolated time
        ...
        ani = FuncAnimation(
            ...
            frames=num_frames,
            init_func=init,
            blit=False, # Blit=True often causes issues with updating text/paths
            interval=30 # milliseconds per frame (e.g., 30ms -> ~33 fps)
        )
...
if __name__ == "__main__":
    ...
    # Use BASE_ORBITAL_SPEED_FACTOR consistently
    orbital_speeds = [np.sqrt(MU / orb.radius) * BASE_ORBITAL_SPEED_FACTOR if orb.radius > 0 else 0 for orb in orbits]
    print(f"Calculated base orbital speeds (sim units): {[f'{s:.4f}' for s in orbital_speeds]}")

    # Add satellites to orbits
    # Orbit 0 (Low)
    sim.add_satellite(orbits[0], np.deg2rad(0), BASE_ORBITAL_SPEED_FACTOR)
    sim.add_satellite(orbits[0], np.deg2rad(120), BASE_ORBITAL_SPEED_FACTOR)
    sim.add_satellite(orbits[0], np.deg2rad(240), BASE_ORBITAL_SPEED_FACTOR)
    # Orbit 1 (Medium)
    sim.add_satellite(orbits[1], np.deg2rad(90), BASE_ORBITAL_SPEED_FACTOR)
    sim.add_satellite(orbits[1], np.deg2rad(270), BASE_ORBITAL_SPEED_FACTOR)
    # Orbit 2 (High)
    sim.add_satellite(orbits[2], np.deg2rad(180), BASE_ORBITAL_SPEED_FACTOR)
